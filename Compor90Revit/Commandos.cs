...
using System.IO;
using static Compor90Revit.Config;
    ...
    public class CmdExtrairDados : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                saveDialog.InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData).ToString() + "\\Autodesk\\REVIT\\Addins\\2018";
            ...
            foreach (Element instancia in instancias)
            {
                ...
                switch (unidade)
                {
                    ...
                    case Config.Unidade.Volume:
                        ...
                        break;
                    case Config.Unidade.Altura:
                        quantidade += instancia.GetParameters("Width")[0].AsDouble();
                        break;
                    case Config.Unidade.Dimensao:
